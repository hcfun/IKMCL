...
class MetaTrainer(Trainer):
    ...
    def train(self):
        ...
        for e in range(self.args.metatrain_num_epoch):
            for batch in self.train_subgraph_dataloader:
                ...
                # batch_ent_emb = self.split_emb(ent_emb, batch_sup_g.batch_num_nodes().tolist())
                #batch_rel_emb = self.split_emb(rel_emb, batch_sup_g.batch_num_nodes().tolist())
                #batch_ent_emb = self.split_emb(ent_emb, batch_sup_g.batch_num_nodes().tolist())
                for batch_i, data in enumerate(batch):
                    que_tri, que_neg_tail_ent, que_neg_head_ent,que_pos_tail_ent,que_pos_head_ent,weight = [d.to(self.args.gpu) for d in data[1:]]
                    ent_emb = sup_g_list[batch_i].ndata['h']

                    #rel_emb=batch_rel_emb[batch_i]
                    #ent_emb=batch_ent_emb[batch_i]
                   # ent_emb2 = que_g_list[batch_i].ndata['h']
                    # kge loss
                    loss = self.get_loss(que_tri, que_neg_tail_ent, que_neg_head_ent, ent_emb,weight)
                    cl_loss1 = self.get_cl_loss(que_tri, que_pos_tail_ent, que_pos_head_ent, ent_emb)

                    batch_loss = loss+batch_loss+cl_loss1
    ...
    def evaluate_valid_subgraphs(self):
        ...
        for batch in self.valid_subgraph_dataloader:
            ...
            batch_sup_g = dgl.batch([get_g_bidir(d[0], self.args) for d in batch]).to(self.args.gpu)
            ent_emb=self.get_ent_emb(batch_sup_g)
