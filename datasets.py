...
class TrainSubgraphDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #求权重
            hr_freq= {key: len(values) for key, values in hr2t.items()}
            tr_freq={key: len(values) for key, values in rt2h.items()}
            subsampling_weight=[]
            for i in que_tri:
                tmp=hr_freq[(i[0],i[1])]+tr_freq[(i[1],i[2])]
                weight=torch.sqrt(1/torch.Tensor([tmp]))
        ...
        que_neg_head_ent = [np.random.choice(np.delete(np.arange(nentity), rt2h[(r, t)]),
                                        self.args.metatrain_num_neg) for h, r, t in que_tri]
        que_pos_head_ent = [np.random.choice(rt2h[(r, t)],
                                             self.args.metatrain_num_pos) for h, r, t in que_tri]
        que_pos_tail_ent = [np.random.choice( hr2t[(h, r)],
                                             self.args.metatrain_num_pos) for h, r, t in que_tri]

        return torch.tensor(sup_tri), torch.tensor(que_tri), \
               torch.tensor(que_neg_tail_ent), torch.tensor(que_neg_head_ent),torch.tensor(que_pos_tail_ent),torch.tensor(que_pos_head_ent),torch.tensor(subsampling_weight)
...
class KGETrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        neg_head_ent = np.random.choice(np.delete(np.arange(self.num_ent), self.rt2h[(r, t)]),
                                        self.num_neg)
        pos_head_ent = np.random.choice(self.rt2h[(r, t)],self.num_neg)
        pos_tail_ent = np.random.choice(self.hr2t[(h, r)],
                                             self.num_neg)
        hr_freq = {key: len(values) for key, values in self.hr2t.items()}
        tr_freq = {key: len(values) for key, values in self.rt2h.items()}

        tmp = hr_freq[(h, r)] + tr_freq[(r, t)]
        weight = torch.sqrt(1 / torch.Tensor([tmp]))
        ...
        pos_tail_ent = torch.from_numpy(pos_tail_ent)
        pos_head_ent = torch.from_numpy(pos_head_ent)


        return pos_triple, neg_tail_ent, neg_head_ent,pos_tail_ent,pos_head_ent,weight
    ...
    def collate_fn(data):
        ...
        pos_tail_ent = torch.stack([_[3] for _ in data], dim=0)
        pos_head_ent = torch.stack([_[4] for _ in data], dim=0)
        weight = torch.stack([_[5] for _ in data], dim=0)
        return pos_triple, neg_tail_ent, neg_head_ent,pos_tail_ent,pos_head_ent,weight
