...
class EntInit(nn.Module):
    def __init__(self, args):
        ...
        self.rel_emb = nn.Parameter(torch.Tensor(args.num_rel*2, args.rel_dim))
    ...
    def forward(self, g_bidir):
        ...
        reduce_func = dgl.function.mean('msg', 'feat')#将msg的消息进行平均，并将结果存在feat中
        g_bidir.update_all(message_func, reduce_func)
        g_bidir.edata.pop('ent_e')
        ent_emb=g_bidir.ndata['feat']
        return ent_emb
